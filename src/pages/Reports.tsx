...
export default function Reports() {
  ...
  const chartData = Object.entries(monthlyData).sort().slice(-6).map(([month, revenue]) => ({
    month: new Date(month + "-01").toLocaleDateString(locale, { month: "short", year: "numeric" }),
    revenue,
  }));
    ...
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="lavage-card p-6 text-center">
          <p className="text-3xl font-bold text-primary">{totalRevenue} {C}</p>
        ...
        <div className="lavage-card p-6 text-center">
          <p className="text-3xl font-bold text-warning">{avgOrder} {C}</p>
          <p className="text-sm text-muted-foreground">{t("reports.avgOrder")}</p>
        </div>
      </div>

      <div className="grid md:grid-cols-2 gap-6">
        <div className="lavage-card p-6">
          <h3 className="text-lg font-bold text-foreground mb-4">{t("reports.monthlyRevenue")}</h3>
          {chartData.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">{t("reports.noDataYet")}</p>
          ) : (
            <ResponsiveContainer width="100%" height={300}>
              <BarChart data={chartData}>
                ...
                <Tooltip formatter={(v: number) => [`${v} ${C}`, t("reports.revenueLabel")]} contentStyle={{ background: 'hsl(220,15%,8%)', border: '1px solid hsl(220,15%,15%)', borderRadius: '0.75rem', color: '#fff' }} />
        ...
        <div className="lavage-card p-6">
          <h3 className="text-lg font-bold text-foreground mb-4">{t("reports.topServices")}</h3>
          {pieData.length === 0 ? (
            <p className="text-center text-muted-foreground py-8">{t("reports.noDataYet")}</p>
          ) : (
            <ResponsiveContainer width="100%" height={300}>
