...
export default function Login() {
  ...
  const handleNameSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data?.error) { toast.error(data.error); setLoading(false); return; }
      if (!data?.access_token || !data?.refresh_token) {
        toast.error(t("auth.unableLogin")); setLoading(false); return;
      }
      const { error: sessErr } = await supabase.auth.setSession({
        access_token: data.access_token,
        refresh_token: data.refresh_token,
  ...
  const handleGoogleSignIn = async () => {
    try {
      const result = await lovable.auth.signInWithOAuth("google", {
        redirect_uri: window.location.origin + (redirectTo || "/post-login"),
      });
      if ((result as any)?.error) {
        toast.error((result as any).error.message || t("auth.loginFailed"));
      }
    } catch (err: any) {
      toast.error(err?.message || t("auth.loginFailed"));
    }
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div className="relative my-2">
              ...
              <div className="relative flex justify-center text-xs">
                <span className="bg-[#060612] px-2 text-muted-foreground">{t("common.or", "أو")}</span>
              </div>
            </div>
            ...
            <Button
              ...
              className="w-full h-12 font-bold bg-white text-black hover:bg-white/90 transition-all flex items-center justify-center gap-2"
            >
